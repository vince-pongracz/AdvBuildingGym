...
class ActionSmoothnessReward(RewardFunction):
    ...
    Per action key, keep a ring buffer of the last ``n_steps`` actions; each step
    take an ``rfft`` over time per dimension. Energy |X[k]|² near the Nyquist bin
    (action flipping every step) marks the oscillation to discourage.

    Modes (``mode``):
    - ``'weighted'`` (default): weight each bin by ``w[k] = (k/(N/2))^freq_exponent``
      (DC→0, Nyquist→1), normalise by the worst case → per-dim penalty in [0, 1]. Smooth, preferred for RL.
    - ``'highband'``: only bins above ``cutoff_bin = round(cutoff_fraction*(N/2))`` count
      (brick-wall high-pass); trends below the cutoff → 0.

    Aggregation: mean over dims per key, then **sum** over keys → ``osc ∈ [0, n_keys]``,
    raw reward ``-osc`` (pure penalty).
    Worst case: a ±1 square wave puts all energy at Nyquist (``|X[N/2]|² = N²``), so N²/dim.
    See ``docs/action_smoothness_spectral.md``.
    Refs: rfft https://numpy.org/doc/stable/reference/generated/numpy.fft.rfft.html ;
    ...
    def get_reward(self, actions: dict, state: dict, next_state: dict, info: dict | None = None) -> float:
        ...
        for key, current_action in actions.items():
            ...
            # time-ordered view: roll by -head so row 0 = oldest, row -1 = newest
        ...
        # Pure-penalty form: raw_reward in [-n_action_keys, 0].
        raw_reward = -osc_total
        return float(self.weight * raw_reward)

    def _spectral_penalty(self, x: np.ndarray) -> float:
        """Per-key spectral penalty in [0, 1] for time-ordered ``x`` (N, D).
        ...
        # worst case per dim: ±1 square wave puts all energy at Nyquist → N²
        worst_per_dim = float(N * N)
        penalty_per_dim = energy_per_dim / worst_per_dim   # shape (D,)
        # mean over dims (comparable across D), clip [0, 1]
