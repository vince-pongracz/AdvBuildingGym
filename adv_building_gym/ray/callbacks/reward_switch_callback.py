...
evaluation) once ``num_episodes_lifetime`` has advanced by at least
``max(swap_every_n_episodes, num_env_runners)`` since the previous swap.
Optionally fires the event-driven exploration bump (shared util) when the
active set / weights change.
"""
...
def create_reward_switch_on_train_result_cb(
    ...
    Args:
        ...
        num_env_runners: Active env_runner count; floor on the swap window.
    ...
    gate = make_swap_gate(
        "RewardSchedule", reward_manager.swap_every_n_episodes, num_env_runners,
    )
    ...
    def on_train_result(*, algorithm, result: dict, **kwargs) -> None:
        ...
        if decision.is_first_fire:
            # Push the initial active reward set without advancing the manager.
