...
def resolve_checkpoint_path(
    ...
    ``models_base/{config_name}/ray/{algorithm}`` for the latest by mtime. A
    missing directory raises an exception rather than broadening the search.
    ...
    if not os.path.isdir(search_base):
        raise FileNotFoundError(
            f"No checkpoint directory for this trial/algorithm: {search_base}. "
