...
class HP(Infrastructure):
    ...
    def exec_action(self, actions, states) -> None:
        ...
        q_hp = float(np.atleast_1d(action)[0]) * self.Q_hp_max

        dT = (0.001
              * (self.timestep / self.mC)
              * (self.K * (float(states["temp_norm"][0]) - float(states["temp_norm_out"][0])) + q_hp))
        self.temp_norm -= dT
        self.temp_norm_change = dT

    def update_state(self, states) -> None:
        new_temp = states["temp_norm"][0] + self.temp_norm_change
        # Clip to observation space bounds and ensure float32
        states["temp_norm"][0] = np.float32(np.clip(new_temp, -1.0, 1.0))
