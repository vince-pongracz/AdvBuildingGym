...
# Created: 2026-07-15 | Version: 1.0
# Description: Submit a SLURM job that runs `run_train_ray.py` for DreamerV3 trials.

# -----------------------------------------------------------------------------
# Usage:
#   sbatch slurm_scripts/slurm_train_ray_dreamerv3.sh [OPTIONS]
#
...
#
# DreamerV3 variant of slurm_train_ray.sh: books fewer CPUs. DreamerV3 runs
# all sampling in-process on the driver — num_env_runners is forced to 0 in
# adv_building_gym/ray/training/resource_setup.py (Ray < 2.53.0 reads env
# spaces off the driver-local runner; https://github.com/ray-project/ray/issues/56749)
# — so CPUs booked for remote env runners would sit idle. Use slurm_train_ray.sh
# for SAC / PPO trials; tools/snapshot/submit_snapshot.py picks the matching
# wrapper from the trial YAML's `algorithm:` key automatically.
#
# Examples:
#   sbatch slurm_scripts/slurm_train_ray_dreamerv3.sh --trial configs/trial_cfgs/v0/trial_cfg_1_dreamerv3.yaml
#
...
#SBATCH --output=slurm_logs/train/slurm-train-ray-dreamerv3-%j.out
#SBATCH --error=slurm_logs/train/slurm-train-ray-dreamerv3-%j.err
#SBATCH --job-name=ray-train-dreamerv3-%j
...
echo "=== Starting Ray DreamerV3 training job ==="
...
#     chmod +x slurm_scripts/slurm_train_ray_dreamerv3.sh
# - Submit:
#     sbatch slurm_scripts/slurm_train_ray_dreamerv3.sh --trial configs/trial_cfgs/v0/trial_cfg_1_dreamerv3.yaml
