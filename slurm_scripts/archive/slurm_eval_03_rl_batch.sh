...
for ALG in "${ALGORITHMS[@]}"; do
  JOB_NAME=$(printf "eval-rl-%02d" "$count")
  echo "Submitting job: $JOB_NAME for algorithm: $ALG"
  sbatch --job-name="$JOB_NAME" slurm_script/slurm_eval_03a_rl_single.sh "$ALG"
  ((count++))
done
...
#     chmod +x slurm_script/slurm_eval_03_rl_batch.sh
#
# - To run the script:
#     ./slurm_script/slurm_eval_03_rl_batch.sh
#
