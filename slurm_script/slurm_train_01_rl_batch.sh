...
# and observation variants using slurm_train_01_rl_batch.sh
# -----------------------------------------------------------------------------
...
echo "[INFO] Checking GPU info via slurm_check_gpu_info.sh"
bash slurm_script/slurm_check_gpu_info.sh
...
for ALG in "${ALGORITHMS[@]}"; do
  for MODE in "${REWARD_MODES[@]}"; do
    ...
    for OBS in "${OBS_VARIANTS[@]}"; do
      ...
      sbatch "$(dirname "$0")/slurm_train_01a_rl_single.sh" "$ALG" "$NUM_ENVS" "$MODE" "$OBS"
      sleep 0.5
    done
  done
done
...
#
# - Each job calls: slurm_train_01_rl_batch.sh <algorithm> <num_envs> <reward_mode> <obs_variant>
# -------------------------------------------------------------------------------
