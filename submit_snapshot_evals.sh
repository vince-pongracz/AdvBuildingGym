...
#
# Per snapshot, two INDEPENDENT eval kinds may be submitted:
...
# A snapshot may get both, one, or neither (neither => skipped).
#
# Note: there is no eval-sb kind in this repo (no run_eval_sb.py / slurm wrapper),
# so SB snapshots are not handled here.
#
# Usage:
...
for snap in "$SNAPSHOTS_DIR"/*/; do
    ...
    if [[ "$did_submit" -eq 0 ]]; then
        echo "  -> SKIP (no Ray checkpoint, no RBC realised in snapshot)"
        n_skipped=$((n_skipped + 1))
    fi
done

echo
echo "Done. eval: $n_eval, eval-rbc: $n_rbc, skipped: $n_skipped"
